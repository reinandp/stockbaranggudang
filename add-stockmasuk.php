...
$query_kategori = "SELECT DISTINCT id_kategori FROM nama_barang";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $insert_querymasuk = "INSERT INTO stock_masuk (id_kategori, kode_barang, nama_barang, nama_supplier, jumlah_masuk, tanggal_masuk) VALUES 
    ('$id_kategori','$kode_barang','$nama_barang','$nama_supplier','$jumlah_masuk','$tanggal_masuk')";
    ...
    // Pernyataan SQL untuk memeriksa apakah kode_barang sudah ada dalam tabel stock_barang
    $query_cek_stock = "SELECT COUNT(*) as count FROM stock_barang WHERE kode_barang = '$kode_barang'";
    $result_cek_stock = $db->query($query_cek_stock);

    if ($result_cek_stock && $result_cek_stock->num_rows > 0) {
        $row_cek_stock = $result_cek_stock->fetch_assoc();
        $count = $row_cek_stock['count'];
    } else {
        $count = 0;
    }

    if ($count > 0) {
        // Kode barang sudah ada dalam tabel stock_barang, update jumlah_stock
        $update_querybarang = "UPDATE stock_barang SET jumlah_stock = jumlah_stock + $jumlah_masuk WHERE kode_barang = '$kode_barang'";
    } else {
        // Kode barang belum ada dalam tabel stock_barang, insert data baru
        $insert_querybarang = "INSERT INTO stock_barang (kode_barang, nama_barang, harga_satuan, jumlah_stock) VALUES ('$kode_barang', '$nama_barang', '$harga_satuan', '$jumlah_masuk')";
    }

    if ($db->query($insert_querymasuk) === TRUE && ($count > 0 ? $db->query($update_querybarang) : $db->query($insert_querybarang))) {
                ...
                <div class="d-flex flex-row flex-column-fluid container">
                    ...
                    <div class="main d-flex flex-column flex-row-fluid">
                        ...
                        LEFT JOIN kategori_barang AS kb ON nb.id_kategori = kb.id";
                                                        ...
                                                        <div class="form-group col-6">
                                                            <label for="exampleSelect4">Nama Supplier</label>
                                                            <select class="form-control" name="nama_supplier" id="exampleSelect4" required>
                                                                <option>Pilih Supplier</option>
                                                                <?php
                                                                $query_supplier = "SELECT nama_supplier FROM supplier";
                                                                ...
                                                                if ($result_supplier && $result_supplier->num_rows > 0) {
                                                                    while ($row_supplier = $result_supplier->fetch_assoc()) {
                                                                        ...
                                                                        echo "<option value=\"$nama_supplier\">$nama_supplier</option>";
                                                                    }
                                                                } else {
        ...
        function validateForm() {
            ...
            for (var i = 0; i < inputs.length; i++) {
                if (inputs[i].value === "" && inputs[i].hasAttribute("required")) {
                    ...
                    return false; // Menampilkan pesan kesalahan dan mencegah pengiriman formulir
                }
            }
            return true; // Mengirim formulir jika semua input diisi
        }
    </script>
    <script>

...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $select_query = "SELECT * FROM stock_keluar WHERE id=?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['jumlah_kg'])) {
        ...
        // Pernyataan SQL untuk mengambil jumlah_stock dan jumlah_kg sebelumnya
        $stock_query = "SELECT sb.jumlah_stock, sk.jumlah_kg FROM stock_barang AS sb
                        INNER JOIN stock_keluar AS sk ON sb.kode_barang = sk.kode_barang
                        WHERE sk.id = '$id'";
        $stock_result = $db->query($stock_query);

        if ($stock_result && $stock_result->num_rows > 0) {
            $row = $stock_result->fetch_assoc();
            $jumlah_stock_sebelumnya = $row['jumlah_stock'];
            $jumlah_kg_sebelumnya = $row['jumlah_kg'];

            if ($jumlah_kg != $jumlah_kg_sebelumnya) {
                // Perbarui jumlah_stock hanya jika jumlah_kg berbeda
                $selisih_kg = $jumlah_kg - $jumlah_kg_sebelumnya;
                $jumlah_stock_baru = $jumlah_stock_sebelumnya - $selisih_kg;
                $update_querybarang = "UPDATE stock_barang SET jumlah_stock = $jumlah_stock_baru WHERE kode_barang = '$kode_barang'";
            ...
            // Pernyataan SQL untuk mengubah data dalam tabel stock_keluar
            $update_querykeluar = "UPDATE stock_keluar SET kode_nota = '$kode_nota', nama_pembeli = '$nama_pembeli', id_kategori = '$id_kategori', kode_barang = '$kode_barang',  nama_barang = '$nama_barang',
            harga_satuan = '$harga_satuan', jumlah_kg = '$jumlah_kg', total_harga = '$total_harga', tanggal_keluar = '$tanggal_keluar', status_bayar = '$status_bayar' WHERE id = '$id'";

            if ($db->query($update_querykeluar) === TRUE) {
                ...
                $db->rollback();
                echo "Error updating stock_keluar: " . $db->error;
            }
        } else {
            echo "Data stock_keluar tidak ditemukan";
        }
                    ...
                    <div class="main d-flex flex-column flex-row-fluid">
                                                ...
                                                <div class="card-body">
                                                    <div class="row">
                                                        <div class="form-group col-6">
                                                            ...
                                                            <input type="text" class="form-control" name="kode_nota" id="kode_nota" value="<?php echo $data['kode_nota']; ?>" onblur="checkKodeNotaAvailability()">
                                                            <span id="kode_nota_message" style="color: red; display: none;">Kode Nota sudah tersedia</span>
                                                        </div>
                                                        <div class="form-group col-6">
                                                            <label for="nama_pembeli">Nama Pembeli</label>
                                                            <input type="text" class="form-control" name="nama_pembeli" id="nama_pembeli" value="<?php echo $data['nama_pembeli']; ?>">
                                                        </div>
                                                        <div class="form-group col-6">
                      ...
                      LEFT JOIN kategori_barang AS kb ON sm.id_kategori = kb.id";
                                                        ...
                                                        <div class="form-group col-6">
                                                            ...
                                                            <input type="text" class="form-control" name="harga_satuan" id="harga_satuan" readonly value="<?php echo $data['harga_satuan']; ?>">
                                                        </div>
                                                        <div class="form-group col-6">
                                                            <label for="jumlah_kg">Jumlah Kg</label>
                                                            <input type="text" class="form-control" name="jumlah_kg" id="jumlah_kg" value="<?php echo $data['jumlah_kg']; ?>">
                                                            ...
                                                            <span id="stock_message" style="color: red; display: none;">Stock tidak tersedia</span>
                                                        </div>
                                                        <div class="form-group col-6">
                                                            <label for="total_harga">Total Harga</label>
                                                            <input type="text" class="form-control" name="total_harga" id="total_harga" readonly value="<?php echo $data['total_harga']; ?>">
                                                        </div>
                                                        <div class="form-group col-6">
                                                            <label class="col-form-label" for="kt_datepicker_3">Tanggal Keluar Barang</label>
                                                            <div class="input-group date">
                                                                <input type="text" class="form-control" readonly value="<?php echo $data['tanggal_keluar']; ?>" id="kt_datepicker_3" />

...
$query_data = "SELECT id, kode_nota, nama_pembeli, kode_barang, nama_barang, harga_satuan, jumlah_kg, total_harga, tanggal_keluar, status_bayar FROM stock_keluar";
...
if (isset($_GET['id'])) {
    ...
    // Query untuk mendapatkan jumlah_kg sebelum menghapus
    $get_kg_query = "SELECT jumlah_kg, kode_barang FROM stock_keluar WHERE id='$id'";
    $result = $db->query($get_kg_query);

    if ($result->num_rows == 1) {
        $row = $result->fetch_assoc();
        $jumlah_kg_sebelum_hapus = $row['jumlah_kg'];
        $kode_barang = $row['kode_barang'];

        // Hapus data dari tabel stock_keluar
        $delete_query = "DELETE FROM stock_keluar WHERE id='$id'";

        if ($db->query($delete_query) === TRUE) {
            // Mengembalikan jumlah_stock ke nilai sebelumnya
            $update_stock_query = "UPDATE stock_barang SET jumlah_stock = jumlah_stock + $jumlah_kg_sebelum_hapus WHERE kode_barang='$kode_barang'";
            if ($db->query($update_stock_query) === TRUE) {
                        ...
                        <div class="card card-custom gutter-b">
                            <div class="card-header flex-wrap py-3">
                                <div class="card-title">
                                    <h3 class="card-label">Data Stock Keluar
                                </div>
                                    ...
                                    <div class="dropdown dropdown-inline mr-2">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right">
                                            ...
                                            <ul class="navi flex-column navi-hover py-2">
                                                ...
                                                <li class="navi-item">
                                                    <a href="generate_csv.php?table=stock_keluar" class="navi-link">
                                ...
                                <table class="table table-bordered table-checkable table-responsive" id="kt_datatable">
                                    ...
                                    <tbody>
                                        ...
                                        while ($data = $result_data->fetch_assoc()) {
                                            ...
                                            echo "<td>" . $data['kode_nota'] . "</td>";
                                            echo "<td>" . $data['nama_pembeli'] . "</td>";
                                            echo "<td>" . $data['kode_barang'] . "</td>";
                                            echo "<td>" . $data['nama_barang'] . "</td>";
                                            echo "<td>Rp " . number_format($data['harga_satuan'], 0, ',', '.') . "</td>";
                                            echo "<td>" . $data['jumlah_kg'] . " kg</td>";
                                            echo "<td>Rp " . number_format($data['total_harga'], 0, ',', '.') . "</td>";
                                            echo "<td>" . $data['status_bayar'] . "</td>";
                                            echo "<td>";
                                            echo '<a href="edit-stockkeluar.php?id=' . $data['id'] . '" class="btn btn-sm btn-clean btn-icon" title="Delete">';
                                            echo '<i class="la la-edit"></i>';
                                            echo '</a>';
                                            echo '<a href="data-stockkeluar.php?id=' . $data['id'] . '" class="btn btn-sm btn-clean btn-icon" title="Delete">';
